<template>
  <div class="container mx-auto px-3 py-5 md:px-6 md:py-8">
    ...
    <div class="hidden gap-8 lg:grid lg:grid-cols-2">
      ...
      <div
        class="flex flex-col rounded-2xl border-2 border-primary/70 bg-ink-800/40">
        <div
          class="flex-1 overflow-y-auto p-6 text-[17px] leading-relaxed text-primary/90">
          ...
          <component :is="'div'" v-else>
            <div v-if="hasCharacteristics" class="space-y-3">
              <div
                ...
                <span class="text-primary/60">{{ formatKey(key) }}</span>
      ...
      <div class="flex flex-col items-center justify-center">
        ...
        <div class="mt-6 flex flex-wrap justify-center gap-3">
          <button
            :class="
              mediaMode === 'photo' ? 'pill-btn !bg-primary/10' : 'pill-btn'
            "
            @click="mediaMode = 'photo'">
            Фото
          </button>
    ...
    <div class="space-y-5 lg:hidden">
      ...
      <div
        ...
        <div class="grid grid-cols-2 gap-2 border-t border-ink-600 p-2.5">
          <button
            class="rounded-lg border px-3 py-2.5 font-display text-sm tracking-wide transition-colors"
            :class="tabClass('characteristics')"
            @click="activeTab = 'characteristics'">
            Характер-ка
          </button>
      ...
      <div v-if="itemCategory" class="text-center text-sm text-primary/60">
        Категория:
        <RouterLink
          :to="{ name: 'CategoryDetail', params: { slug: itemCategory.slug } }"
    ...
    <div
      v-if="itemCategory"
      class="mt-6 hidden text-sm text-primary/60 lg:block">
      Категория:
      <RouterLink
        :to="{ name: 'CategoryDetail', params: { slug: itemCategory.slug } }"
        class="text-primary hover:text-accent">
...
const formatKey = (key) => keyMap[key] || key;
...
const itemCategory = computed(() => {
  ...
  return (
    store.getters["categories/getCategoryById"](props.item.category_id) || null
  );
});
